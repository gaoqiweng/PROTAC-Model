...
def filter_frodock(cpu, lig_locate_num, target_smi, rec_smi):
    ...
    if rec_smi != 'none':
        pre.pdb_AssignBondOrder('rec_lig.pdb', rec_smi, 'rec_lig.sdf')
    else:
        pre.obabel_convert_format('pdb', 'rec_lig.pdb', 'sdf', 'rec_lig.sdf')
    pre.obabel_convert_format('sdf', 'rec_lig.sdf', 'sdf', 'rec_lig_H.sdf', addH=True)
    pre.obabel_convert_format('sdf', 'rec_lig_H.sdf', 'pdb', 'rec_lig_H.pdb')
    if target_smi != 'none':
        pre.pdb_AssignBondOrder('target_lig.pdb', target_smi, 'target_lig.sdf')
    else:
        pre.obabel_convert_format('pdb', 'target_lig.pdb', 'sdf', 'target_lig.sdf')
    pre.obabel_convert_format('sdf', 'target_lig.sdf', 'sdf', 'target_lig_H.sdf', addH=True)
    ...
    for i in range(1, pdb_num + 1):
        para_list.append([i, rec_site_info_list, lig_site, lig_locate_num,
              filepath_rec_lig_1, filepath_rec_lig_2, target_smi, rec_smi])
    pool = Pool(cpu)
...
def filtering(para_list):
    pdb_num = para_list[0]
    rec_site_info_list = para_list[1]
    lig_site = para_list[2]
    lig_locate_num = para_list[3]
    filepath_rec_lig_1 = para_list[4]
    filepath_rec_lig_2 = para_list[5]
    target_smi = para_list[6]
    rec_smi = para_list[7]
    filepath_vina = 'vina'
    ...
    # get the interface residue
    content_interface = interface_frodock(rec_site_info_list,
                                          lig_site, target_addH_pdb)
    ...
    if filter_num != 'none':
        ...
        if lig_locate_num == 1:
            ...
            if int(num_confor) > 0:
                ...
                os.system(
                    'grep ATOM %s >> %s' % (target_addH_pdb, model_nolig_pdb))
                # obenergy and vina
